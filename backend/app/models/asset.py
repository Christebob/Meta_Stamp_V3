...
DANGEROUS_EXTENSIONS: List[str] = [
    ...
    ".exe", ".bin", ".sh", ".app", ".msi", ".iso", ".dmg",  # Executables
...
class Asset(BaseModel):
    ...
    user_id: str = Field(
        ...
        min_length=1,
        max_length=100,
        description="Reference to owning user's ID"
    )
    ...
    s3_key: str = Field(
        ...
        min_length=1,
        max_length=1024,
        description="Object key in S3/MinIO storage"
    )
    ...
    upload_status: UploadStatus = Field(
        ...
        description="Current upload/processing state"
    )
    
    processing_status: Optional[ProcessingStatus] = Field(
        default=None,
        description="Fingerprint processing state (None if not started)"
    )
    
    error_message: Optional[str] = Field(
        default=None,
        max_length=1000,
        description="Error details if upload/processing failed"
    )
    
    # Relationships
    fingerprint_id: Optional[str] = Field(
        default=None,
        description="Reference to associated fingerprint document"
    )
    ...
    def validate_upload_status(cls, v: Any) -> UploadStatus:
        ...
        if isinstance(v, str):
            ...
            try:
                ...
                raise ValueError(
                    f"Invalid upload status '{v}'. Allowed: {allowed}"
                )
    ...
    @classmethod
    def validate_url_source(cls, v: Optional[str]) -> Optional[str]:
        """
...
class AssetResponse(BaseModel):
    ...
    fingerprint_id: Optional[str] = Field(None, description="Fingerprint reference")
    url_source: Optional[str] = Field(None, description="Source URL")
    ...
    def from_asset(cls, asset: Asset) -> "AssetResponse":
        ...
        return cls(
            ...
            file_type=asset.file_type.value if isinstance(asset.file_type, FileType) else asset.file_type,
            file_size=asset.file_size,
            file_size_mb=asset.get_file_size_mb(),
            mime_type=asset.mime_type,
            s3_key=asset.s3_key,
            s3_bucket=asset.s3_bucket,
            upload_status=asset.upload_status.value if isinstance(asset.upload_status, UploadStatus) else asset.upload_status,
            processing_status=asset.processing_status.value if isinstance(asset.processing_status, ProcessingStatus) else asset.processing_status,
