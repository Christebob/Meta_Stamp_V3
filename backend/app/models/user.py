...
class User(BaseModel):
    ...
    email: EmailStr = Field(
        ...,
        description="Validated email address for account identification"
    )
    ...
    hashed_password: Optional[str] = Field(
        default=None,
        description="Bcrypt hashed password for local authentication fallback"
    )
    
    is_active: bool = Field(
        default=True,
        description="Account status flag (inactive accounts cannot login)"
    )
    ...
    followers_count: int = Field(
        default=0,
        ge=0,
        description="Total followers/subscribers across platforms"
    )
    ...
    primary_platform: Optional[str] = Field(
        default=None,
        max_length=50,
        description="Primary creator platform (youtube, vimeo, etc.)"
    )
    
    # Metadata storage
    metadata: Dict[str, Any] = Field(
        default_factory=dict,
        description="Additional user metadata and settings"
    )
    ...
    @classmethod
    def validate_primary_platform(cls, v: Optional[str]) -> Optional[str]:
        """
    ...
    def update_platform_metrics(
        ...
        content_hours: Optional[float] = None,
...
class UserCreate(BaseModel):
    ...
    auth0_id: Optional[str] = Field(
        default=None,
        description="Auth0 ID for SSO registration"
    )
...
class UserResponse(BaseModel):
    ...
    primary_platform: Optional[str] = Field(None, description="Primary platform")
...
class UserUpdate(BaseModel):
    ...
    followers_count: Optional[int] = Field(
        default=None,
        ge=0,
        description="Updated follower count"
    )
    
    content_hours: Optional[float] = Field(
        default=None,
        ge=0.0,
        description="Updated content hours"
    )
    
    total_views: Optional[int] = Field(
        default=None,
        ge=0,
        description="Updated total views"
    )
