...
class Transaction(BaseModel):
    ...
    asset_id: Optional[str] = Field(
        default=None,
        max_length=100,
        description="Associated asset ID for earnings attribution"
    )
...
class WalletBalance(BaseModel):
    ...
    pending_earnings: Decimal = Field(
        default=Decimal("0.00"),
        description="Pending AI Touch Value™ earnings"
    )
    ...
    total_paid_out: Decimal = Field(
        default=Decimal("0.00"),
        description="Lifetime total payouts"
    )
    
    # Payout tracking
    last_payout_at: Optional[datetime] = Field(
        default=None,
        description="Timestamp of most recent payout"
    )
    ...
    updated_at: datetime = Field(
        default_factory=lambda: datetime.now(timezone.utc),
        description="UTC timestamp of last wallet update"
    )
    ...
    def is_payout_eligible(self, minimum_threshold: Optional[Decimal] = None) -> bool:
        """
